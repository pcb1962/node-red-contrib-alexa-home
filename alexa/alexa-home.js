...
//        var sensorsMatch = /^\/api\/(\w*)\/sensors\/([\w\-]*)/.exec(request.url);
    ...
    function AlexaHomeNode(config) {
        ...
        if (!node.controller) {
            RED.log.error(node.name + " - Could not get a Alexa Home Controller - node is not functional!")
    ...
    AlexaHomeNode.prototype.handleEvent = function(config,msg) {
    ...
    RED.nodes.registerType("alexa-temperature", AlexaTemperatureNode);

    function AlexaTemperatureNode(config) {
        AlexaHomeNode.call(this,config);
    }
    utils.inherits(AlexaTemperatureNode, AlexaHomeNode);
    AlexaTemperatureNode.prototype.handleEvent = function(config,msg){
        var node = this;
        if (msg == null || msg.payload === null || msg.payload === undefined) {
            node.status({
                fill: "red",
